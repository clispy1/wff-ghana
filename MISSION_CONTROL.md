...
- **Next.js 16 upgrade (16 Aug 2026)** — upgraded from 15.5.19 to 16.3.1, React 19.2.8. Turbopack is now the default for both `next dev` and `next build`.
  ...
  - **`output: 'standalone'` removed from `next.config.ts`.** It previously masked a Windows-local EPERM symlink issue; on Vercel under Next 16 it instead broke the *production* deploy — `next build` succeeded locally but Vercel's post-build step failed with `ENOENT: .next/next-server.js.nft.json`, because `standalone`'s file-tracing layout no longer matches what Vercel's own build pipeline expects. `standalone` is meant for self-hosting (Docker, bare Node) — Vercel does its own output tracing and never needed it. All 46 routes still build cleanly without it.
